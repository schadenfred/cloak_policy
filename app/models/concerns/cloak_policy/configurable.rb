    ...
    def descendants(children=nil, descendants=nil)
      ...
      children.each do |child| 
        ...
        when (child.class.name.eql?('CloakPolicy::Vector') && child.bottom_level?)
          unless child.scored_settings.empty?
            ...
            child.intents.each do |intent| 
              intents << { id: intent.id, name: intent.name, icon: intent.icon, description: intent.description, weight: intent.weight, child_type: "intent",  selected:  false}
            end
            array << { name: child.name, icon: child.icon, description: child.description, description: child.description, children: descendants(child.scored_settings), intents: intents }         
        ...
        when child.class.name.eql?('CloakPolicy::Vector')
          intents = []
          child.intents.each do |intent| 
            intents << { id: intent.id, name: intent.name, icon: intent.icon, weight: intent.weight, description: intent.description, child_type: "intent", selected: false }
          end    
        ...
        when child.class.name.eql?('CloakPolicy::Choice')
          array << { name: child.name, icon: child.setting.icon, weight: child.weight, size: 100, child_type: 'choice', selected: false }
        end
      end
      array
    end
