...
class ScorableTest < ActiveSupport::TestCase
  ...
  Given(:recommendations_platform) { recommendations_platforms(:one) }
  Given(:platform)                 { platforms(:facebook) }
  Given(:setting)                  { settings(:one) }
  Given(:vector)                   { vectors(:privacy) }

  scorables = %w(choice preference recommendations_platform platform setting)

  scorables.each do |scorable|

    Given(:subject) { scorable.classify.constantize.new }
